...
- separar artículo bruto y artículo destilado
...
| `pipeline_run_id` | UUID FK | Referencia a `pipeline_runs.id` |
| `raw_article_id` | UUID FK | Referencia a `raw_articles.id` |
| `decision` | VARCHAR | `new`, `duplicate`, `review` |
| `matched_article_id` | UUID NULL | Si hubo coincidencia previa |
...
| `raw_article_id` | UUID FK UNIQUE | Referencia al artículo bruto origen |
| `source_id` | UUID FK | Medio de origen |
| `section` | VARCHAR | `Local`, `El Bierzo`, etc. |
| `slug` | VARCHAR UNIQUE | URL amigable |
| `original_title` | TEXT | Titular fuente |
...
| `source_name_snapshot` | VARCHAR | Nombre de fuente congelado |
| `original_url` | VARCHAR | URL de referencia |
...
| Estado | Significado |
|---|---|
| `new` | Debe seguir el pipeline |
| `duplicate` | Ya existe o es redundante |
